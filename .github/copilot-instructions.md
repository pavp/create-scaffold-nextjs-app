...
This is the **create-nextlane-app** CLI tool and its companion template for bootstrapping modern Next.js frontend applications at Nextlane. Follow these instructions when generating or editing code in this project.
...
- **Analytics**: MixPanel (`useInitAnalytics`) | **Feedback**: Screeb (`useInitFeedback`)
...
- Translation keys are managed in **t8r** (internal Nextlane tool).
...
```typescript
// In root layout
useInitAnalytics(); // requires NEXT_PUBLIC_MIXPANEL_TOKEN
useInitFeedback(); // requires NEXT_PUBLIC_SCREEB_WEBSITE_ID
```
