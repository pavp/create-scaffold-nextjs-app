...
async function downloadTemplate(targetDir, projectData = {}) {
  const tempDir = path.join(require('os').tmpdir(), `scaffold-template-${Date.now()}`);
...
// Main CLI function
async function main() {
  console.log(`${colors.bright}${colors.cyan}🚀 Create Scaffold App${colors.reset}`);
  console.log(`${colors.blue}Bootstrap a modern frontend application with Scaffold's template${colors.reset}\n`);
